<template>
	...
	<view class="index-page">
		...
		<view class="main-container">
			<view class="title">过关证明(2/3)</view>
			<view class="tips">请保持拍摄时光线良好，避免证件反光</view>
			<view class="imgs-wrap">
				<image class="img" v-for="n in imgsBlock" :key="n" :src="imgs[n-1] || '/static/image/过关证明.png'" mode="aspectFill"
...
<script>
	...
	export default {
		...
		methods: {
				...
				uni.chooseImage({
					success: async res => {
						...
						const {
							resourceKey,
							url
						} = await this.$app.uploadOSS(path, 1, `openacc/${this.$app.getData('applyId') || 0}/passproof`)
						// 显示已上传成功的图片
			...
			async nextStep() {
				const res = await this.$app.http('account/openAccount/offline/passProofSubmit', {
					applyId: this.$app.getData('applyId'),
					passProofUrl: this.imgs.filter(a => a),
				})
